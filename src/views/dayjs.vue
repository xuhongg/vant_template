<template>
    <div class="bj">
        <div class="column">
            ...
            <span>十年前{{ date1 }}</span>
            <span>十年后{{ date2 }}</span>
            <van-button type="info" @click="showDatePicker = !showDatePicker">选择年月日</van-button>
        </div>

        <van-cell-group class="cellGroup">
            <van-cell center title="选择日期date" :value="JSON.stringify(currentDate)" is-link />
            <van-cell center title="选择日期string" :value="chooseDate" is-link />
        </van-cell-group>
        <van-popup :overlay='flag' class="popUp" v-model="showDatePicker" position="bottom">
            <van-datetime-picker @confirm="confirmDate" @cancel="showDatePicker = false" v-model="currentDate"
